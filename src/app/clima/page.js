...
export default function ClimaPage() {
  ...
  const [climateArticles, setClimateArticles] = useState([]);
  ...
  useEffect(() => {
    async function fetchClimateArticles() {
      try {
        ...
        console.error('Erro ao carregar notícias do clima:', err);
      } finally {
        setLoadingArticles(false);
      }
    }
    fetchClimateArticles();
  }, []);
  ...
  useEffect(() => {
    ...
    async function carregarClima() {
      ...
      try {
        ...
        if (!ignore) setError('Não foi possível carregar os dados meteorológicos no momento.');
      } finally {
    ...
    <main className="main-content" style={{ maxWidth: '1080px', margin: '0 auto', padding: '24px 16px' }}>
      ...
      {error ? (
        ...
        <>
          ...
          {/* REPORTAGENS E ANÁLISES CLIMÁTICAS */}
          <div style={{ marginTop: '48px', marginBottom: '32px' }}>
            <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ fontSize: '20px', fontWeight: '700', color: 'var(--text)', display: 'flex', alignItems: 'center', gap: '8px', margin: 0 }}>
                <span className="material-icons-extended" style={{ color: '#0f9d58' }}>eco</span>
                Reportagens e Análises do Clima & Agro
              </h2>
            </div>

            {loadingArticles ? (
              <div style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)' }}>
                Carregando matérias climáticas...
              </div>
            ) : climateArticles && climateArticles.length > 0 ? (
              <div className="news-grid">
                {climateArticles.map((article, index) => (
                  <Link key={article.id} href={`/artigo/${article.slug}`} className="article-card">
                    {getOptimizedImageUrl(article.image_url) ? (
                      ...
                      <div className="article-card-img" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--gn-border)' }}>
                    ...
                    <div className="article-card-content">
                      <div>
                        <div className="article-card-category" style={{ color: article.categories?.color_code || '#0f9d58' }}>
                          {article.categories?.name || 'Clima tempo'}
                        </div>
                        <h3 className="article-card-title" style={{ fontSize: '1.15rem', marginTop: '6px', marginBottom: '6px' }}>
                          {article.title}
                        </h3>
                        <p className="article-card-summary" style={{ fontSize: '0.95rem' }}>
              ...
              <div style={{ textAlign: 'center', padding: '36px', background: 'var(--card)', borderRadius: '12px', border: '1px solid var(--border)', color: 'var(--text-muted)' }}>
                <span className="material-icons-extended" style={{ fontSize: '36px', color: 'var(--border)', marginBottom: '8px', display: 'block' }}>article</span>
                <p style={{ margin: 0, fontSize: '14px' }}>Nenhuma reportagem do clima publicada no momento.</p>
              </div>
            )}
          </div>
        </>
