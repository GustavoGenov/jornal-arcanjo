...
export default async function AdminDashboard() {
  // 1. Buscar todos os artigos com categorias e posições de destaque
  const { data: allArticles, count: articlesCount } = await supabase
    .from('articles')
    .select('id, title, slug, views, category_id, published, created_at, content, summary, author_name, featured_position, categories(id, name, slug)', { count: 'exact' })
