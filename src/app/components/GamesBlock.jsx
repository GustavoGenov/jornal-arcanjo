  ...
  return (
    <div id="passatempos" style={{ marginTop: '40px', border: '1px solid var(--gn-border)', borderRadius: '12px', background: 'var(--gn-surface)', overflow: 'hidden' }}>
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--gn-border)', display: 'flex', gap: '16px', background: 'var(--gn-search-bg)', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => handleTabChange('sudoku')} style={{ background: 'none', border: 'none', color: activeTab === 'sudoku' ? '#1a73e8' : 'var(--gn-text-secondary)', fontWeight: activeTab === 'sudoku' ? '600' : '400', cursor: 'pointer', fontSize: '14px', borderBottom: activeTab === 'sudoku' ? '2px solid #1a73e8' : '2px solid transparent', paddingBottom: '4px' }}>Sudoku</button>
        <button onClick={() => handleTabChange('termo')} style={{ background: 'none', border: 'none', color: activeTab === 'termo' ? '#1a73e8' : 'var(--gn-text-secondary)', fontWeight: activeTab === 'termo' ? '600' : '400', cursor: 'pointer', fontSize: '14px', borderBottom: activeTab === 'termo' ? '2px solid #1a73e8' : '2px solid transparent', paddingBottom: '4px' }}>Termo</button>
